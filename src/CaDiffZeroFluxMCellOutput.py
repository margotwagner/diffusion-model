...
def ca_snare_finder(mcell_viz_dir, timepoint):
    ...
    z_s = -0.24550  # using refrac box location
    ...
    for seed in sorted(os.listdir(mcell_viz_dir)):
        ...
        seed_data_dir = os.path.join(
            mcell_viz_dir, seed, "Scene.ascii.{}.dat".format(timepoint)
        )
...
mcell_dir = (
    "/home/bartol/mcell_projects_2/margot/rect_zeroflux_nocalbpmca_files/"
    "mcell/output_data/viz_data_ascii"
)

# get time points
timepts = [
    scene.split(".")[2] for scene in os.listdir(os.path.join(mcell_dir, "seed_00001"))
]
timepts.sort()
...
plt.fill_between(
    timepts_int,
    np.array(ca_conc_all_mean) - np.array(ca_conc_all_std),
    np.array(ca_conc_all_mean) + np.array(ca_conc_all_std),
    alpha=0.2,
    color="k",
)
